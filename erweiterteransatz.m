...
[x, fs] = audioread('F:\Bachelorarbeit\Music\mixture11.wav'); %orginal Music    
x = x(:, 1);
...
sep_stft_erste_per=medfilt2(abs(stft_erste), [48 1]); %median filter 500Hz
...
beta_erste = 3.5; %je höhere Separation Faktor deutlicher Filter aber schwingt 3.5
%mask_sep_stft_erste_har=((sep_stft_erste_har./(sep_stft_erste_per+epsilon_erste))>beta_erste);
...
%figure(5)
%mask_sep_stft_erste_har=(sep_stft_erste_har>10);
%imagesc(t,f,mask_sep_stft_erste_har);
%set(gca,'YDir','normal');
...
figure(20)
[stft_zweite, f, t_stft] = stft(istft_masked_rest, wlen_per, hop_per, nfft_per, fs);
...
sep_stft_zweite_res_per=medfilt2(abs(stft_zweite), [3 1]);
...
%zweite Masking (binary mask nach Schwellenwert)
%figure(23)
%masked_sep_stft_zweite_per=(sep_stft_zweite_res_per>0.5);
...
beta_zweite = 2; %ideal 1.2
masked_sep_stft_zweite_per=((sep_stft_zweite_res_per./(sep_stft_zweite_har+epsilon_zweite))>beta_zweite);
imagesc(t,f,masked_sep_stft_zweite_per);
set(gca,'YDir','normal');
